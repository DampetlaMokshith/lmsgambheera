...
export const SlideEffect: React.FC<EffectRendererProps> = ({
  ...
  // Handle reduced motion delete timer
  useEffect(() => {
    if (prefersReducedMotion && allowDelete && isActive) {
      const t = window.setTimeout(
        () => onDeleteComplete?.(),
        Math.max(200, pauseAfterTypeMs)
      );
  ...
  }, [onDeleteComplete, pauseAfterTypeMs, allowDelete, prefersReducedMotion, isActive]);
